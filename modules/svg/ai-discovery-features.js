...
import { discoverability } from '../../data';
...
export function svgAiDiscoveryFeatures(projection, context, dispatch) {
    ...
    function hasAiFeatures(d) {
        return d.properties.rapid_available === true;
    }
    ...
    function drawData(selection) {        
        ...
        layer = selection.selectAll('.layer-map-ai-discovery-features')
            .data(_enabled && true ? [0] : []);
        ...
        layer = layer.enter()
            .append('g')
            .attr('class', 'layer-map-ai-discovery-features')
            .merge(layer);
        ...
        if (context.map().zoom() < context.minEditableZoom()) {
            geoData = getFeatures(discoverability);
            geoData = geoData.filter(getPath);
            geoData = geoData.filter(hasAiFeatures); 
        ...
        var datagroups = layer
            .selectAll('g.ai-discovery-featuresgroup')
            .data(['fill', 'shadow', 'stroke']);

        datagroups = datagroups.enter()
            .append('g')
            .attr('class', function(d) { return 'ai-discovery-featuresgroup ai-discovery-featuresgroup-' + d; })
            .merge(datagroups);
